...
# 1. Remove the Facebook button completely
t = re.sub(r'<button class="social" id="fbBtn"[^>]*>.*?</button>\s*', '', t, flags=re.DOTALL)
print("✅ Removed Facebook button.")

# 2. Replace the old script with the robust onAuthStateChange version
...
function msg(t, kind){ var m = document.getElementById("formMsg"); m.textContent = t || ""; m.className = "msg " + (kind || ""); m.style.display = t ? "block" : "none"; }

async function finishLogin(session){
  var token = session && session.access_token;
  if(!token) return false;
  try{
    var r = await fetch("/api/login", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ access_token: token }) });
    var d = await r.json();
    if(!d.error){ location.replace("/app"); return true; }
    msg("Login failed: " + (d.error || "server error"), "err");
  }catch(e){ msg("Login failed: " + e.message, "err"); }
  return false;
}

// ROBUST OAUTH HANDLING: automatically catches the Google redirect session
(function() {
  var c = client();
  if (c) {
...
print("✅ Replaced script block with robust Google OAuth handler.")